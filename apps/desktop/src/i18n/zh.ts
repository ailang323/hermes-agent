...
export const zh: Translations = {
  ...
  kanban: {
    ...
    orchestration: {
      ...
      loadingMode: "正在加载模式…",
      autoModeTitle:
        "编排：自动 — 调度器会在每次 tick 自动拆解新的待分类任务。点击切换到手动模式。",
      manualModeTitle:
        "编排：手动 — 待分类任务会留在待分类列，直到你在卡片上点击 ⚗ 拆解。点击切换到自动模式。",
      configureTitle:
        "配置看板编排器（编排器配置、默认负责人、自动拆解、配置描述）",
      reload: "重新加载",
      orchestratorProfile: "编排器配置",
      defaultProfile: "（默认：{profile}）",
      resolved: "当前：{profile}",
      orchestratorHelp:
        "在扇出后拥有根任务（会被唤醒来判断完成情况）。它不决定任务如何拆分；拆解模型请在 auxiliary.kanban_decomposer 中配置。",
      defaultAssignee: "默认负责人",
      mode: "编排模式",
      autoDecompose: "自动拆解待分类任务",
      autoDecomposeHelp: "调度器会自动拆解新的待分类任务。",
      manualDecomposeHelp: "待分类任务会留在待分类列，直到你点击 ⚗ 拆解。",
      loading: "加载中…",
      profileDescriptions: "配置描述",
      profileDescriptionsHelp:
        "描述会指导拆解器的路由。点击 ⚗ 自动生成，或手动编辑后保存。",
      noProfiles: "没有安装任何配置。",
      defaultTag: "（默认）",
      autoReview: "自动 — 待检查",
      noDescriptionWarning: "⚠ 无描述",
      descriptionPlaceholder: "这个配置擅长什么？",
      saveDescriptionTitle: "将上方描述保存为用户编写内容",
      save: "保存",
      saving: "保存中…",
      ...
      autoGenerateFailed: "自动生成失败：{message}",
      unknownError: "未知错误",
    },
    columnLabels: {
      triage: "待分类",
  ...
  updates: {
    ...
    managedDecisionTitle: '更新候选需要确认',
    managedConflictBody: '隔离候选存在冲突。已安装分支和应用均未更改。请先处理列出的文件，再重新准备候选。',
    managedReviewBody: '检测到上游可能已包含一项或多项本地定制。请确认是否移除对应本地补丁后再继续。',
    managedConflictFiles: '冲突文件',
    managedRecommendations: '优化建议',
    managedUpstreamEquivalentRecommendation: '此功能采用上游实现。',
    managedUpstreamEquivalentReason: '重放后的候选已不再需要此本地补丁：',
    managedWorktree: '冲突工作树',
    managedReport: '详细报告',
    managedConfirmationTitle: '安装已验证的更新？',
    managedConfirmationBody: '此候选已通过配置的检查。请核对提交和不可变制品哈希，然后才会修改正式分支或应用。',
    managedCandidateCommit: '候选提交',
    managedArtifactHash: '制品 SHA-256',
    managedInstallVerified: '安装已验证更新',
    managedAcceptRecommendations: '采用上游实现并继续',
    managedResumeConflict: '已解决，继续验证',
    managedCancel: '取消',
    errorTitle: '更新未完成',
