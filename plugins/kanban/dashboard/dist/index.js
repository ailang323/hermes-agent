  ...
  function OrchestrationPanel() {
    ...
    const loadAll = useCallback(function () {
      Promise.all([
        ...
        setMsg({ ok: false, text: tx(t, "orchestration.failedToLoad", "Failed to load: {message}", { message: err.message || String(err) }) });
      });
    }, []);
    ...
    const saveSettings = function (patch) {
      ...
      return SDK.fetchJSON(`${API}/orchestration`, {
        ...
        setMsg({ ok: false, text: tx(t, "orchestration.saveFailed", "Save failed: {message}", { message: err.message || String(err) }) });
      });
    };
    ...
    const saveProfileDescription = function (name, description) {
      ...
      return SDK.fetchJSON(`${API}/profiles/${encodeURIComponent(name)}`, {
        ...
        loadAll();
        setMsg({ ok: true, text: tx(t, "orchestration.descriptionSaved", "Description saved for {name}.", { name: name }) });
      }).catch(function (err) {
    ...
    const autoGenerateDescription = function (name, overwrite) {
      ...
      return SDK.fetchJSON(`${API}/profiles/${encodeURIComponent(name)}/describe-auto`, {
        ...
        if (res && res.ok) {
          loadAll();
          setMsg({ ok: true, text: tx(t, "orchestration.autoGenerated", "Auto-generated description for {name}.", { name: name }) });
        } else {
          setMsg({
            ok: false,
            text: tx(t, "orchestration.autoGenerateFailed", "Auto-generate failed: {message}", { message: (res && res.reason) || tx(t, "orchestration.unknownError", "unknown error") }),
          });
        }
      }).catch(function (err) {
        setMsg({ ok: false, text: tx(t, "orchestration.autoGenerateFailed", "Auto-generate failed: {message}", { message: err.message || String(err) }) });
      }).then(function () {
        setBusy(function (b) {
          const next = Object.assign({}, b); delete next[name]; return next;
        });
    ...
    const modePillTitle = settings === null
      ...
      : (autoOn
          ? tx(t, "orchestration.autoModeTitle", "Orchestration: Auto — the dispatcher decomposes new triage tasks automatically every tick. Click to switch to Manual (pre-PR behavior).")
          : tx(t, "orchestration.manualModeTitle", "Orchestration: Manual — triage tasks stay in triage until you click ⚗ Decompose on each card. Click to switch to Auto."));
    const modePill = h("button", {
      ...
      tx(t, "orchestration.label", "Orchestration: "),
    ...
    if (!expanded) {
      return h("div", { className: "flex items-center gap-3 text-xs" },
        ...
        h("button", {
          ...
          title: tx(t, "orchestration.configureTitle", "Configure the kanban orchestrator (profile picker, default assignee, auto-decompose, profile descriptions)"),
        }, headerLabel),
      );
    }
      ...
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        settings ? h("div", { className: "grid gap-3 sm:grid-cols-3" },
          h("div", { className: "flex flex-col gap-1" },
            ...
            h(Select, Object.assign({
              ...
              h(SelectOption, { value: "" },
                tx(t, "orchestration.defaultProfile", "(default: {profile})", { profile: settings.active_profile || "default" })),
              profileOptions,
            ),
            h("div", { className: "text-[10px] text-muted-foreground" },
              tx(t, "orchestration.resolved", "Resolved: {profile}", { profile: settings.resolved_orchestrator_profile || "default" })),
            h("div", { className: "text-[10px] text-muted-foreground" },
          ...
          h("div", { className: "flex flex-col gap-1" },
            ...
            h("div", { className: "text-[10px] text-muted-foreground" },
              tx(t, "orchestration.resolved", "Resolved: {profile}", { profile: settings.resolved_default_assignee || "default" })),
          ),
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs text-muted-foreground" },
              tx(t, "orchestration.mode", "Orchestration mode")),
            ...
            h("div", { className: "text-[10px] text-muted-foreground" },
              settings.auto_decompose
                ? tx(t, "orchestration.autoDecomposeHelp", "The dispatcher decomposes new triage tasks automatically.")
                : tx(t, "orchestration.manualDecomposeHelp", "Triage tasks stay in triage until you click ⚗ Decompose.")),
          ),
  ...
  function ProfileDescriptionRow(props) {
    ...
    return h("div", { className: "flex flex-col gap-1 border-l-2 pl-2",
      ...
      h("div", { className: "flex items-center gap-2 text-xs" },
        ...
        p.is_default ? h("span", { className: "text-[10px] text-muted-foreground" }, tx(t, "orchestration.defaultTag", "(default)")) : null,
        p.description_auto && p.description
          ? h("span", { className: "text-[10px] text-yellow-600" }, tx(t, "orchestration.autoReview", "auto — review"))
          : null,
        !p.description
          ? h("span", { className: "text-[10px] text-yellow-600" }, tx(t, "orchestration.noDescriptionWarning", "⚠ no description"))
          : null,
      ),
      h("div", { className: "flex items-center gap-2" },
        h(Input, {
          ...
          placeholder: tx(t, "orchestration.descriptionPlaceholder", "What is this profile good at?"),
        ...
        h(Button, {
          ...
          title: tx(t, "orchestration.saveDescriptionTitle", "Save the description above as user-authored"),
        }, busy === "save" ? tx(t, "orchestration.saving", "Saving…") : tx(t, "orchestration.save", "Save")),
        h(Button, {
          onClick: function () { props.onAuto(p.name, true); },
          size: "sm",
          disabled: !!busy,
          title: tx(t, "orchestration.autoDescriptionTitle", "Auto-generate a description from this profile's skills and model"),
        }, busy === "auto" ? tx(t, "orchestration.generating", "Generating…") : tx(t, "orchestration.autoButton", "⚗ Auto")),
      ),
    );
  }
